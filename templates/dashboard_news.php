...
<div id="dashboard-rss">
	...
	<div class="">	
		...
		<h2><?php _e( 'Latest Blog Posts from G1T:', 'glow' ); ?></h2>
	
		<?php
		
		// Get a SimplePie feed object from the specified feed source.
		$rss = fetch_feed( 'http://status.god1st.cloud/feed' );
		...
		<ul>
		    <?php if ( $maxitems == 0 ) : ?>
		        <li><?php _e( 'No items', 'glow' ); ?></li>
		        		    <?php else : ?>
		        <?php // Loop through each feed item and display each item as a hyperlink. ?>
		        <?php foreach ( $rss_items as $item ) : ?>
		            <li>
		                <a href="<?php echo esc_url( $item->get_permalink() ); ?>"
		                    target="GLOW_alerts" title="<?php printf( __( 'Posted %s', 'glow' ), $item->get_date('j F Y | g:i a') ); ?>">
