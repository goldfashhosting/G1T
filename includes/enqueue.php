...
if ($parent_theme_author == 'WooThemes') {
	...
	function bb_woo_enqueue() {
		...
		if ( $G1TGLOWPlugin->get_setting('disable_flex') != "yes") {
...
function bamboo_css() {
	...
	global $G1TGLOWPlugin;
	if ( $G1TGLOWPlugin->get_setting('disable_fontawe') != "yes") {
	...
	if (defined('G1T_INSTALLED')) {

		if (G1T_VERSION_NUM > 0.1 && G1T_VERSION_NUM < 1.2 ) {
		...
		if (G1T_VERSION_NUM > 0.1 && G1T_VERSION_NUM < 1 ) {
		...
		elseif (G1T_VERSION_NUM > 1 && G1T_VERSION_NUM < 1.2 ) {
...
function register_bamboo_js() {
	...
	global $G1TGLOWPlugin;
	if ( $G1TGLOWPlugin->get_setting('disable_modernizr') != "yes") {
...
if (defined('G1T_INSTALLED') && G1T_VERSION_NUM < 1.2 ) {
	
	function legacy_print_bamboo_foot() {
		wp_print_scripts('modernizr');
	}
	
	function legacy_print_bamboo_head() {
		if (defined('G1T_INSTALLED')) {
			wp_print_scripts('G1T-js');
		}
	}
	
	wp_register_script('G1T-js',  get_stylesheet_directory_uri() . '/assets/js/jsgeneral.js', array('jquery'), BB_VERSION, true);
	add_action('wp_head', 'legacy_print_bamboo_head');
	add_action('wp_footer', 'legacy_print_bamboo_foot');
...
if ( $G1TGLOWPlugin->get_setting('disable_prettyphoto') != "yes" ) {
