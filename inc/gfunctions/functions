...
function my_allowed_meta_keys( $args ) {
	...
	$allowed_meta_keys = array( 'ui_licensekey_api', 'gender' );
...
add_filter( 'rest_user_query', 'my_allowed_meta_keys' );
...
function acme_save_user_permissions( $user_id ) {
  // If the current user doesn't have administrative priveleges, exit.
	...
	$suser_contact_method['messenger_support'] = __( 'FB Messenger Support', 'text_domain', true);
	$suser_contact_method['messenger_support_name'] = __( 'FB Messenger Name', 'text_domain', true);
	$suser_contact_method['ui_licensekey_api'] = __( 'User License Key', 'text_domain', true);
	$suser_contact_method['ui_goldpay_account'] = __( 'AutoBilling+', 'text_domain' );
	$suser_contact_method['bbui_goldpay_account'] = __( 'BackUp AutoBilling+', 'text_domain' );
	$suser_contact_method['gsetaubpd'] = __( 'Billing Date', 'text_domain' );
	$suser_contact_method['sim'] = __( 'SIM', 'text_domain' );
	$suser_contact_method['tsid'] = __( 'SID', 'text_domain' );
	$suser_contact_method['ttid'] = __( 'TTID', 'text_domain' );
		return $suser_contact_method;
	}
...
add_filter( 'user_contactmethods',  'acme_save_user_permissions' );



function GOS_ui_api_objects( $user_contact_method ) {
	
	$user_contact_method['ui_mobile'] = __( 'Mobile Number', 'text_domain', true);
	$user_contact_method['ui_pin_api'] = __( 'Access Pin', 'text_domain', true);
...
define('YOUR_SPECIAL_SECRET_KEY', '593b52f5058684.11445629'); //Rename this constant name so it is specific to your plugin or theme.
$rServer = ''. $_SERVER['SERVER_NAME'] .'';
// This is the URL where API query request will be sent to. This should be the URL of the site where you have installed the main license manager plugin. Get this value from the integration help page.
define('YOUR_LICENSE_SERVER_URL', 'https://goldfash.com/i'); //Rename this constant name so it is specific to your plugin or theme.
...
$bar = get_user_option( 'ui_licensekey_api', $userid );
	...
	$fbpic = get_user_option('fbpic', $userid );
...
<h3> </h3>
    ...
    <table class="form-table">
 	...
 	<tr>
 	 <th><label for"fbpic"> Your FB Pic</label></th>
 	 
 	 	<td>
 	 	<?
 	 		if(empty($fbpic)){
 		echo('Is not available at the moment');
 		}else{
 		echo('<img src="'. $fbpic .'" />');
 		}
 		?>
    ...
    <option value="@vtext.com">PagePlus</option>
        ...
        <tr>
<th><label for="dnp_cklist">Products</label></th>
<td><input type="checkbox" name="prod_gmwp" <?php if('on'==esc_attr(get_the_author_meta('prod_gmwp',$user->ID ))) echo 'checked="checked"'; ?>/> GoldManagement WordPress+<br />
 
<input type="checkbox" name="prod_shortcodes" <?php if('on'==esc_attr(get_the_author_meta('prod_shortcodes',$user->ID ))) echo 'checked="checked"'; ?>/> Shortcodes+<br />
 
<input type="checkbox" name="prod_gold2gk" <?php if('on'==esc_attr(get_the_author_meta('prod_gold2gk',$user->ID ))) echo 'checked="checked"'; ?>/> Gold2GK Communications<br />
 
<input type="checkbox" name="prod_onesupport" <?php if('on'==esc_attr(get_the_author_meta('prod_onesupport',$user->ID ))) echo 'checked="checked"'; ?>/> Support<br />
 
<input type="checkbox" name="prod_rafcolinks" <?php if('on'==esc_attr(get_the_author_meta('prod_rafcolinks',$user->ID ))) echo 'checked="checked"'; ?>disabled/> RaFco Links+ (Coming Soon)<br />
 
<input type="checkbox" name="prod_devapi" <?php if('on'==esc_attr(get_the_author_meta('prod_devapi',$user->ID ))) echo 'checked="checked"'; ?> disabled/> Dev Api+ (Coming Soon)<br />
</td>
</tr>
...
if (empty($bar) && (!empty($_GET['create_license'])) ){
		  ...
		  /*** Mandatory data ***/ 
// Post URL 
$postURL = 'https://goldfash.com/i'; 
// The Secret key 
$secretKey = '593b52f50585d8.43573658'; 
...
function my_save_extra_profile_fields( $user_id ) {
  ...
  //  update_usermeta( absint( $user_id ), 'gsetapilc', wp_kses_post( $_POST['gsetapilc'] ) );
    ...
    update_usermeta( absint( $user_id ), 'prod_gmwp', wp_kses_post( $_POST['prod_gmwp'] ) );
    update_usermeta( absint( $user_id ), 'prod_shortcodes', wp_kses_post( $_POST['prod_shortcodes'] ) );
    update_usermeta( absint( $user_id ), 'prod_gold2gk', wp_kses_post( $_POST['prod_gold2gk'] ) );
    update_usermeta( absint( $user_id ), 'prod_onesupport', wp_kses_post( $_POST['prod_onesupport'] ) );
    update_usermeta( absint( $user_id ), 'prod_rafcolinks', wp_kses_post( $_POST['prod_rafcolinks'] ) );
    update_usermeta( absint( $user_id ), 'prod_devapi', wp_kses_post( $_POST['prod_devapi'] ) );

    update_usermeta( absint( $user_id ), 'fbpic', wp_kses_post( $_POST['fbpic'] ) );
